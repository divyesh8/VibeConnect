-- Prefer an opposite-gender partner when one is available, but do not leave a
-- user waiting when another eligible person is already in the same-mode queue.
-- Existing row locks and SKIP LOCKED preserve atomic matchmaking under load.
