...
create table if not exists public.match_proposals (
  ...
  source_room_id uuid references public.chat_rooms(id) on delete set null,
...
alter table public.match_proposals add column if not exists source_room_id uuid references public.chat_rooms(id) on delete set null;
create index if not exists match_proposals_source_room_idx on public.match_proposals (source_room_id, status, expires_at) where source_room_id is not null;
