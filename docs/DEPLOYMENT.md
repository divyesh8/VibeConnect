...
6. Configure `TURN_CREDENTIALS_URL` for a provider that returns short-lived `iceServers`, or set the server-only `TURN_URL`, `TURN_USERNAME`, and `TURN_CREDENTIAL` coturn fallback. This is required for reliable calls between carrier/mobile networks and Wi-Fi; STUN alone cannot traverse every NAT. Do not restore `NEXT_PUBLIC_TURN_*`. Set `WEBRTC_FORCE_RELAY=true` only in development to run a relay-only test.
